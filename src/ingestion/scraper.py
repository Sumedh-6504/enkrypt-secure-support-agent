...
async def fetch_and_parse(url: str, client: httpx.AsyncClient) -> dict:
    ...
    try:
        ...
        return {"url": url, "content": clean_markdown, "status": "success"}
...
async def run_scraper(target_urls: list[str] | None = None):
    ...
    try:
        with open(doc_path, "w", encoding="utf-8") as f:  # noqa: ASYNC230
            for page in scraped_data:
                if page["status"] == "success":
                    f.write(f"\n\n### SOURCE: {page['url']} ###\n\n")
                    f.write(page["content"])
    except Exception:  # noqa: BLE001
        print("Warning: Could not write combined docs file — skipping.")
